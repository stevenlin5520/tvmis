    ...
    <table class="table table-condensed" style="margin-top:8px;">
        ...
        <c:forEach var="item" items="${pager.list}" varStatus="status">
            <tr>
                ...
                <td align="center">${item.channelSort}</td>
                <td align="center">${item.remark}</td>
                <td align="center"><fmt:formatDate value="${item.createTime}" type="both"></fmt:formatDate> </td>
                <td align="center">
                    <div class="layui-btn-group">
                        <button type="button" class="layui-btn layui-btn-sm " onclick="addOrEdit('${item.channelId}')">
                            <i class="layui-icon">&#xe642;</i>
                        </button>
                        <button type="button" class="layui-btn layui-btn-sm layui-btn-danger" onclick="deleted('${item.channelId}')">
                            <i class="layui-icon">&#xe640;</i>
                        </button>
                    </div>
                </td>
...
<script>
        ...
        laypage.render({
            ...
            jump: function(obj,first){
                ...
                if((!first && (lastPage==undefined ? 1 : lastPage != obj.curr || lastLimit==undefined ? 1 : lastLimit != obj.limit))){
                    ...
                    location.replace('${rootPath}/channel/list?page='+obj.curr+'&limit='+obj.limit);
                    console.log(232323)
                }
            }
        });
    ...
    function addOrEdit(id){
        layer.open({
            ...
            time: 0, //0秒后自动关闭
            anim: 2,
            content: ['${rootPath}/channel/edit?id='+id, 'no'], //iframe的url，no代表不显示滚动条
    ...
    function deleted(id){
        layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
            ...
            $.post('${rootPath}/channel/deleted', {id:id}, function(res){
                console.log(res)
                layer.close(loading)
                if(!res.state){
                    layer.open({
