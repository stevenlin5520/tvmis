    ...
    <table class="table table-condensed" style="margin-top:8px;">
        ...
        <c:forEach var="item" items="${pager.list}" varStatus="status">
            <tr>
                ...
                <td align="center">
                    <c:if test="${item.userSex==1}">男</c:if>
                    <c:if test="${item.userSex==2}">女</c:if>
                    <c:if test="${item.userSex==3}">不详</c:if>
                </td>
                <td align="center">${item.userAddress}</td>
                <td align="center"><fmt:formatDate value="${item.userBirthday}" type="date"></fmt:formatDate></td>
                <td align="center">${item.userEmail}</td>
                <td align="center">
                    <c:if test="${item.userType==1}">录入员</c:if>
                    <c:if test="${item.userType==2}">审核员</c:if>
                    <c:if test="${item.userType==3}">供应商</c:if>
                    <c:if test="${item.userType==9}">管理员</c:if>
                </td>
                <td align="center"><fmt:formatDate value="${item.createTime}" type="both"></fmt:formatDate> </td>
                <td align="center">
                    <div class="layui-btn-group">
                        <button type="button" class="layui-btn layui-btn-sm " onclick="addOrEdit('${item.userId}')">
                            <i class="layui-icon">&#xe642;</i>
                        </button>
                        <button type="button" class="layui-btn layui-btn-sm layui-btn-danger" onclick="deleted('${item.userId}')">
                            <i class="layui-icon">&#xe640;</i>
                        </button>
                    </div>
                </td>
...
<script>
        ...
        laypage.render({
            ...
            jump: function(obj,first){
                ...
                if((!first && (lastPage==undefined ? 1 : lastPage != obj.curr || lastLimit==undefined ? 1 : lastLimit != obj.limit))){
                    ...
                    location.replace('${rootPath}/user/list?page='+obj.curr+'&limit='+obj.limit);
                    console.log(232323)
                }
            }
        });
    ...
    function addOrEdit(id){
        layer.open({
            ...
            time: 0, //0秒后自动关闭
            anim: 2,
            content: ['${rootPath}/user/edit?id='+id, 'no'], //iframe的url，no代表不显示滚动条
    ...
    function deleted(id){
        layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
            ...
            $.post('${rootPath}/user/deleted', {id:id}, function(res){
                console.log(res)
                layer.close(loading)
                if(!res.state){
                    layer.open({
