    ...
    <table class="table table-condensed" style="margin-top:8px;">
        ...
        <c:forEach var="item" items="${pager.list}" varStatus="status">
            <tr>
                ...
                <td align="center">${item.sysKey}</td>
                <td align="center">${item.sysValue}</td>
                <td align="center">${item.sysDesc}</td>
                <td align="center"><fmt:formatDate value="${item.sysTime}" type="both"></fmt:formatDate> </td>
                <td align="center">
                    <div class="layui-btn-group">
                        <button type="button" class="layui-btn layui-btn-sm " onclick="addOrEdit('${item.sysId}')">
                            <i class="layui-icon">&#xe642;</i>
                        </button>
                        <button type="button" class="layui-btn layui-btn-sm layui-btn-danger" onclick="deleted('${item.sysId}')">
                            <i class="layui-icon">&#xe640;</i>
                        </button>
                    </div>
                </td>
...
<script>
        ...
        laypage.render({
            ...
            jump: function(obj,first){
                ...
                if((!first && (lastPage==undefined ? 1 : lastPage != obj.curr || lastLimit==undefined ? 1 : lastLimit != obj.limit))){
                    ...
                    location.replace('${rootPath}/system/list?page='+obj.curr+'&limit='+obj.limit);
                    console.log(232323)
                }
            }
        });
    ...
    function addOrEdit(id){
        layer.open({
            ...
            time: 0, //0秒后自动关闭
            anim: 2,
            content: ['${rootPath}/system/edit?id='+id, 'no'], //iframe的url，no代表不显示滚动条
    ...
    function deleted(id){
        layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
            ...
            $.post('${rootPath}/system/deleted', {id:id}, function(res){
                console.log(res)
                layer.close(loading)
                if(!res.state){
                    layer.open({
